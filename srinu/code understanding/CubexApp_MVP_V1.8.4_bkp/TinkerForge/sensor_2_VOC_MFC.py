...
from TinkerForge.Bricklet_ID_Configuration import mfc4_serial_port
...
if __name__ == "__main__":
    ...
    rc = mfc_sfc6.connect(mfc4_serial_port, baudrate=115200)
    if rc != None:
        print(rc)
        exit(0)
    ...
    while True:
        ...
        if choice == "1":
            mfc_sfc6.setpoint_percentage(mfc4_serial_port, 0)
            # Wait until flow stabilizes within 0.002
            for ii in range(10):
                if abs( mfc_sfc6.read_average_value(mfc4_serial_port, 5) - flow)>0.002:
                    time.sleep(0.5)
                else:
                    break
            print(f'flow = {mfc_sfc6.read_average_value(mfc4_serial_port, 5)}')
        elif choice == "2":
        
            # Set Flow Rate to 30% of Max Flow
            mfc_sfc6.setpoint_percentage(mfc4_serial_port, 30)
            print(f"Flow Rate Set to {target_flow} SCCM (30%)")

            # Wait for flow to stabilize
            time.sleep(2)
            print(f'Raw Flow = {mfc_sfc6.measure_raw_flow(mfc4_serial_port)}')
            print(f'flow = {mfc_sfc6.read_average_value(mfc4_serial_port, 5)}')
        elif choice == "3":
        
            # Set Flow Rate to 50% of Max Flow
            mfc_sfc6.setpoint_percentage(mfc4_serial_port, 50)
            ...
            mfc_sfc6.setpoint_percentage(mfc4_serial_port, 100)
