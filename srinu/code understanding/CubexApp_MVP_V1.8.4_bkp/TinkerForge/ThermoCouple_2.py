...
ThermoCouple-1 Unit test application.
These APIs can be reused for testing the ThermoCouple-1 which
...
from TinkerForge.Bricklet_ID_Configuration import ThermoCouple2_UID
...
# ------------------------------
def cb_tubing_temperature(temperature):
    """
    Callback when the thermocouple measures a new temperature.
    """
    temp_c = temperature / 100.0
    integrate.set_tubing_temperature(temp_c)
    print(f"ThermoCouple  {ThermoCouple2_UID} Temperature: {temp_c:.2f} °C")
    

def tc2_initialize_thermocouple(ipcon):
    """
    ...
    """
    thermocouple = BrickletThermocoupleV2(ThermoCouple2_UID, ipcon)
    
    # Register temperature callback to function cb_tubing_temperature
    thermocouple.register_callback(thermocouple.CALLBACK_TEMPERATURE, cb_tubing_temperature)
...
def tc2_reinitialize(uid, device_id, enum_type, ipcon):
    global  reinit
    if device_id == BrickletThermocoupleV2.DEVICE_IDENTIFIER:
        if enum_type in (IPConnection.ENUMERATION_TYPE_CONNECTED, IPConnection.ENUMERATION_TYPE_AVAILABLE):
            ...
            if reinit:
                ...
                tc2_initialize_thermocouple(ipcon)
            reinit = True

def tc2_read_ambient_temperature(thermocouple):
    """
    ...
    try:
        ...
        print(f"Error in {ThermoCouple2_UID} : {e}")
        temp = 0.0
...
def tc2_start_continuous_reading(thermocouple):
    """
...
def tc2_set_target_temperature(thermocouple):
    """
...
def tc2_main():
    ...
    thermocouple = tc2_initialize_thermocouple(ip_con)

    while True:
        print("\n🔹 Thermocouple Test Application Options:")
        ...
        if choice == "1":
            tc2_read_ambient_temperature(thermocouple)
        elif choice == "2":
            tc2_start_continuous_reading(thermocouple)
        elif choice == "3":
            tc2_set_target_temperature(thermocouple)
        elif choice == "4":
