...
class HeaterController:
    ...
    def _apply_name_based_defaults(self) -> None:
        ...
        if self.name == "SAMPLE_HEATER":
            ...
            print(f"Applied SAMPLE_HEATER defaults: rate_alpha={self.rate_alpha}, rate_deadband={self.rate_deadband}, min_hold_sec={self.min_hold_sec}, startup_margin={self.startup_margin}")
            ...
            self.startup_margin = max(self.hysteresis, 0.8)
            print(f"Applied COIL_HEATER defaults: rate_alpha={self.rate_alpha}, rate_deadband={self.rate_deadband}, min_hold_sec={self.min_hold_sec}, startup_margin={self.startup_margin}")
        else:
            self.rate_alpha = 0.25
            self.rate_deadband = 0.004
            self.min_hold_sec = 5.0
            self.startup_margin = max(self.hysteresis, 1.0)
            print(f"Applied generic heater defaults: rate_alpha={self.rate_alpha}, rate_deadband={self.rate_deadband}, min_hold_sec={self.min_hold_sec}, startup_margin={self.startup_margin}")
    ...
    def update(self, current_temp: float, required_temp: float) -> bool:
        ...
        if current_temp >= required_temp:
            print(f"Hard stop at target: current_temp={current_temp:.2f} >= required_temp={required_temp:.2f}")
            self._set_heater_state(HEATER_OFF, now_time)
            self._log_state(required_temp=required_temp, current_temp=current_temp, rate=rate, predicted_off_temp=predicted_off_temp)
            return self.heater_state
        ...
        if self.heater_state == HEATER_ON:
            if current_temp >= predicted_off_temp:
                if self._can_change_state(now_time):
                    print(f"OFF decision while heating: current_temp={current_temp:.2f} >= predicted_off_temp={predicted_off_temp:.2f}")
                    self._set_heater_state(HEATER_OFF, now_time)
        ...
        if current_temp <= turn_on_temp:
            if self._can_change_state(now_time):
                print(f"ON decision while cooling: current_temp={current_temp:.2f} <= turn_on_temp={turn_on_temp:.2f}")
    ...
    def _log_state(self, required_temp: float, current_temp: float, rate: float, predicted_off_temp: float) -> None:
        """
        ...
        """
        readable_time = time.strftime("%I:%M:%S %p", time.localtime())
        now_time = time.monotonic()
        print(
            f"[{readable_time}] "
            f"name={self.name}, "
            f"temp={current_temp:.2f}, "
            f"target={required_temp:.2f}, "
            f"rate={rate:.3f}, "
            f"predicted_off={predicted_off_temp:.2f}, "
            f"heater={'ON' if self.heater_state else 'OFF'}, "
            f"elapsed={now_time - self.last_state_change_time:.2f}, "
            f"min_hold={self.min_hold_sec:.2f}"
        )